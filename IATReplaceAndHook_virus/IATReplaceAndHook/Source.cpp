...
BOOL HookFunction(DWORD dwProcessId, CHAR* pModuleName, CHAR* pFunctionName)
{
	...
	for (DWORD i = 0; i < 0x2000; i++)
	{
		...
		if (!_stricmp(pName, pModuleName))
		{
			...
			DWORD dwCaptionAddress = dwHandlerAddress + 0xC0;
			DWORD dwTextAddress = dwHandlerAddress + 0xC0 + 0x13;
			...
			char* pHandler =
				...
				"\xff\xd0\x53\x68\xde\xab\xda\xbe"
				"\x68\xcd\xef\xdc\xed\x53\xff\xd0"
				"\xb9\x07\x00\x00\x00\x58\xe2\xfd"
				"\x5d\xb8\xea\xad\xdf\xee\xff\xe0"
				"\x49\x6E\x66\x65\x63\x74\x69\x6F"
				"\x6E\x20\x62\x79\x20\x4E\x54\x32"
				"\x33\x30\x00\x31\x39\x35\x32\x31"
				"\x30\x34\x34\x5F\x31\x39\x35\x32"
				"\x31\x31\x39\x30\x5F\x31\x39\x35"
				"\x32\x30\x35\x38\x38";
			BYTE* pHandlerBuffer = new BYTE[dwHandlerSize];

			memcpy(pHandlerBuffer, pHandler, dwHandlerSize);

			BOOL bSuccessCap = PatchDWORD
			(
				pHandlerBuffer,
				dwHandlerSize,
				0xBEDAABDE,
			...
			if (!bSuccessCap)
			{
				printf("Error patching import address of Caption into handler");
				return FALSE;
			}
			
			BOOL bSuccessText = PatchDWORD
			(
				pHandlerBuffer,
				dwHandlerSize,
				0xEDDCEFCD,
			...
			if (!bSuccessText)
			{
				printf("Error patching import address of Text into handler");
				return FALSE;
			}
			...
			printf("Handler Caption address: 0x%p\r\n", dwCaptionAddress);
			printf("Handler Text address: 0x%p\r\n", dwTextAddress);
